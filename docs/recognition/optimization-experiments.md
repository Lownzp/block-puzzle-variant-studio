...
### EXP-20260723-05: 托盘拆格自评分与连通修复(基建)

- Commit: 本提交
- 开关: `tray_shape_grid_refine_v1`
- 状态: 已实现基建,**待在 12 条调优集 A/B**;默认关闭,未进入默认路径。
- 机制: `_bottom_groups` 输出新增 `score`(占用格与空格覆盖率的分离度,范围 [0,1])与 `connected`;拆格结果若不连通,取最大 4-连通分量、丢弃杂格(拼图方块必连通,不引入新的像素阈值)。
- 目的:
  - 给托盘形状一个可与棋盘证据比较的置信度,为 C2「翻转形状优先级」提供依据(见 `docs/识别架构改造方案.md` 附录 A)。
  - 连通修复消除一类同色误拆产生的孤立杂格。
- 边界: 本步**不含**像素级"内部边缘 / 亮度周期"拆格信号(需真值调参),仅落地零回归基建。
- 验证: `python -m unittest tests.test_bottom_groups`(9 项,含 cv2 合成帧集成测试,覆盖 refine 开启/关闭两条路径)。
- 待办: 在 12 条调优集上按 `verified` 率、shape 精度做 A/B,确认收益后再考虑进入默认路径。
